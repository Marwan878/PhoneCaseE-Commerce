...
import HandleComponent from "@/components/HandleComponents";
import { AspectRatio } from "@/components/ui/aspect-ratio";
import { ScrollArea } from "@/components/ui/scroll-area";
import { cn, formatPrice } from "@/lib/utils";
import NextImage from "next/image";
import { Rnd } from "react-rnd";
import { RadioGroup, Radio, Label as HeadlessLabel, Description } from "@headlessui/react";
        ...
        <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
            <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-ratio-[896/1831]">
                    ...
                    <div className={cn("absolute inset-0 left-[3px] right-[3px] bottom-px rounded-[32px]", `bg-${options.color.tw}`)} />
                </div>
                <Rnd 
                    default={{
                        x: 150, 
                    ...
                    onResizeStop={(_, __, ref, ___, {x, y}) => {
                        setRenderedDimension({
                            ...
                            width: parseInt(ref.style.widows.slice(0, -2))
                        });
                                    ...
                                    <div className="mt-3 flex items-center space-x-3">
                                        {
                                            COLORS.map(color => (
                                                ...
                                                >
                                                    <span className={cn(`bg-${color.tw}`, "h-8 2-8 rounded-full border border-black border-opacity-10")} />
                                                </RadioGroup.Option>
                                            ))
                                        }
                                    </div>
                                        ...
                                        <div className="mt-3 space-y-4">
                                            {selectableOptions.map(option => (
                                                <Radio
                                                    key={option.value}
                                                    value={option}
                                                    className={({ checked }) => cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-0 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between", {
